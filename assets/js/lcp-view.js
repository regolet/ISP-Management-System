...
function fetchLcpData(id) {
    const apiUrl = `../api/lcp.php?id=${id}`;
...
function loadLcpPorts(lcpId) {
    ...
    fetch(`../api/lcp.php?action=ports&id=${lcpId}`)
        .then(response => {
...
function loadLcpClients(lcpId) {
    document.getElementById('lcpClients').innerHTML = '<div class="alert alert-info">Client information loading is not yet implemented.</div>';
