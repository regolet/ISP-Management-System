...
// If the plan doesn't exist, redirect to the plans page
if (!$plan) {
    header('Location: ../../plans.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($speed_mbps) || empty($price) || empty($billing_cycle)) {
        ...
        function update_plan($id, $name, $description, $speed_mbps, $price, $billing_cycle, $is_active) {
            global $db;
            $query = "UPDATE plans SET name = :name, description = :description, speed_mbps = :speed_mbps, price = :price, billing_cycle = :billing_cycle, is_active = :is_active WHERE id = :id";
            $stmt = $db->prepare($query);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        ...
        $result = update_plan($id, $name, $description, $speed_mbps, $price, $billing_cycle, $is_active);


        if ($result) {
            // Redirect to the plans page with success message
                ...
                <div class="card-body">
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="description" name="description"><?php echo htmlspecialchars($plan['description']); ?></textarea>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="billing_cycle" name="billing_cycle" required>
                                ...
                                <option value="annually" <?php echo ($plan['billing_cycle'] == 'annually') ? 'selected' : ''; ?>>Annually</option>
                            </select>
                        </div>
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" <?php echo ($plan['is_active']) ? 'checked' : ''; ?>>
